...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_request_schedule'])) {
    ...
    if (!empty($event_date) && strtotime($event_date) < strtotime($min_allowed_date)) {
        $msg = "Bawal mag-request kung mas mababa sa 5 araw bago ang petsa ng gagamiting venue/schedule. Mangyaring pumili ng petsa na hindi bababa sa 5 araw mula ngayon ($min_allowed_date).";
        ...
        <form method="POST" action="" class="ajax-schedule-form bg-light p-3 rounded-3 border">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="date" name="event_date" class="form-control form-control-sm" required value="<?= date('Y-m-d', strtotime('+5 days')) ?>" min="<?= date('Y-m-d', strtotime('+5 days')) ?>">
                </div>
                <div class="col-md-3">
