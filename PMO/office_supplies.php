...
// AJAX Handler para sa pag-submit ng Office Request
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['request_supply'])) {
    ...
    if (!empty($item_ids) && is_array($item_ids)) {
        $request_group_id = 'REQ-' . date('YmdHis') . '-' . rand(100, 999);
        $stmt = $conn->prepare("INSERT INTO supply_requests (request_group_id, user_id, requisitioner_name, department, item_id, quantity, purpose, date_needed) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        try {
            ...
            if ($inserted_count > 0) {
                $conn->commit();
                echo json_encode(['status' => 'success', 'message' => "Matagumpay na naisumite ang iyong Office Supply Request ($request_group_id)!"]);
                exit;
            } else {
...
<nav class="navbar navbar-expand-lg navbar-dark navbar-ecommerce sticky-top">
    <div class="container-fluid px-4">
        <a class="navbar-brand fw-bold text-white d-flex align-items-center" href="user_dashboard.php">
            ...
            <div class="lh-1 ms-1">
                <span class="fs-5 d-block fw-extrabold tracking-tight">OFFICE SUPPLIES STORE</span>
            </div>
        </a>
        <div class="d-flex align-items-center gap-2">
            <a href="maintenance_supplies.php" class="btn btn-outline-light btn-sm rounded-pill px-3"><i class="bi bi-tools me-1"></i> Switch to Maintenance</a>
            <a href="home.php" class="btn btn-light btn-sm rounded-pill px-3"><i class="bi bi-house me-1"></i> Home</a>
        </div>
    </div>
</nav>
    ...
    <div class="hero-banner">
        <div class="row align-items-center">
            <div class="col-lg-7 mb-3 mb-lg-0">
                <span class="badge bg-primary text-white fw-extrabold mb-2 px-3 py-1 rounded-pill text-uppercase">Office Supply Portal</span>
                <h2 class="mb-2">Select Office Supplies</h2>
            </div>
            <div class="col-lg-5">
                ...
                <div id="office-grid" class="row g-3">
                    <?php if(empty($office_items)): ?>
                        <div class="col-12 text-center py-5 bg-white rounded-3 border">
                            ...
                            <h6 class="fw-bold text-dark mb-0">NO Office supply available at this moment.</h6>
                        </div>
                    <?php endif; ?>
            ...
            <div class="col-lg-4">
                <div class="card cart-card">
                    <div class="cart-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0 fw-bold"><i class="bi bi-cart3 me-2"></i>Office Request Cart</h5>
...
function goToCheckoutPage() {
    window.location.href = 'place_order.php?type=office';
}
</script>
</body>
</html>